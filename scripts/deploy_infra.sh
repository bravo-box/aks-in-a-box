...
set_cloud_region() {
  ...
  echo
  read -rp "Enter the Azure region for deployment (e.g., eastus, westus or usgovvirginia): " LOCATION
  while [[ -z "$LOCATION" ]]; do
...
create_or_use_keyvault_and_storage() {
  ...
  if [[ "$CREATE_PREDEPLOY" =~ ^[Yy]$ ]]; then
    echo
    create_or_use_keyvault
    create_or_use_storage_account
  else
    echo
...
get_key_vault() {
  local KEYVAULT_ID=$(az keyvault show -n "$KV_NAME" -g "$RESOURCE_GROUP" --query "id" -o tsv)
  return $KEYVAULT_ID
}

get_storage_account() {
  local STORAGE_ACCOUNT_ID=$(az storage account show -n "$SA_NAME" -g "$RESOURCE_GROUP" --query "id" -o tsv)
  return $STORAGE_ACCOUNT_ID
}

get_deployer_principal_id() {
  local DEPLOYER_PRINCIPAL_ID=$(az ad signed-in-user show --query id -o tsv)
  return $DEPLOYER_PRINCIPAL_ID
}
...
get_uami_and_assign_roles() {
  # --- GET UAMI DETAILS ---
    ...
    # --- GET RESOURCE IDs ---
    STORAGE_ACCOUNT_ID=$(get_storage_account)
    KEYVAULT_ID=$(get_key_vault)
